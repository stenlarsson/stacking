...
namespace Tetatt.GamePlay
{
    ...
    class PlayField
    {
        ...
        public void Draw(SpriteBatch spriteBatch, int x, int y)
        {
            ...
            spriteBatch.Draw(
                background,
                new Vector2(x - 16, y - 16), // Adjust for the frame
                Color.White);
            spriteBatch.End();
            ...
            for (int i = firstBlockFirstVisibleRow; i < numBlocks; i++)
            {
                ...
                if (block != null)
                {
                    ...
                    spriteBatch.Draw(
                        ...
                        new Rectangle(
                            (i % width) * blockSize + x,
                            (i / width - firstVisibleRow) * blockSize + y + (int)scrollOffset,
                            blockSize,
                            blockSize),
            ...
            spriteBatch.Draw(
                marker,
                new Vector2(
                    (markerPos % width) * blockSize + x - 4,
                    (markerPos / width - firstVisibleRow) * blockSize + y + (int)scrollOffset - 5),
                Color.White);
            spriteBatch.End();
        }
