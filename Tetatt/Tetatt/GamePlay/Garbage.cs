...
namespace Tetatt.GamePlay
{
    class Garbage : Block
    {
        ...
        private GarbageBlock gb;
	    private int nextGraphic;
	    private bool wantToDrop;

        private AnimFrame[] flashFrames;

        public Garbage(GarbageType garbageType, BlockType typeAfterPop, GarbageBlock gb)
            : base(garbageType == GarbageType.Evil ? BlockType.EvilGarbage : BlockType.Garbage,
            BlockState.Idle,
            null,
            false,
            new Anim(GarbageBlock.graphicsSingle))
        {
            this.typeAfterPop = typeAfterPop;
            this.gb = gb;
            nextGraphic = GarbageBlock.graphicsDisabled;
            wantToDrop = false;

            flashFrames = new AnimFrame[] {
                new AnimFrame((int)type + GarbageBlock.tileFlash1Offset, 1),
                new AnimFrame((int)type + GarbageBlock.tileFlash2Offset, 4)
            };
        }
        ...
        protected override void ChangeState(BlockState newState)
        {
            ...
            switch (newState)
            {
                ...
                case BlockState.Flash:
                    ...
                    break;
                case BlockState.Pop:
                    anim = new Anim((int)type + GarbageBlock.tileFlash1Offset);
                ...
                case BlockState.Pop3:
                    anim = new Anim(nextGraphic == GarbageBlock.graphicsDisabled ? (int)typeAfterPop : nextGraphic);
                    nextState = BlockState.Dead;
                    stateDelay = dieOffset;
                    break;
                case BlockState.Dead:
                    if (nextGraphic != GarbageBlock.graphicsDisabled)
                    {
                        nextGraphic = GarbageBlock.graphicsDisabled;
